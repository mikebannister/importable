          ...
          context "#import!" do
            ...
            context "existing uploaded spreadsheet" do
              ...
              it "should redirect back to the import view if import is successful and params[:return_to] is set to import" do
                multi_spreadsheet.stubs(:import!).returns(true)
                post :create, type: 'foo', current_step: 'choose_worksheet', spreadsheet_id: multi_spreadsheet.id, return_to: 'import', :use_route => :importable
                response.should redirect_to("/importable/foo")
              end
